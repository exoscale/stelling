...
func (m *Migrations) Migrate(ctx context.Context, conn *sqlite.Conn, targetVersion uint64) (err error) {
	...
	if uint64(len(m.upScripts)) < targetVersion {
		return fmt.Errorf("Target version %d is higher than max migration version %d", targetVersion, len(m.upScripts))
	}
	...
	if uint64(len(m.upScripts)) < version {
		return fmt.Errorf("Database version %d is higher than max migration version %d", version, len(m.upScripts))
	}
	...
	return setDbVersion(conn, targetVersion)
}
